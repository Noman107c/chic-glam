...
export default function InventoryPage() {
  ...
  const filteredProducts = products.filter(p => {
    ...
    const matchesFilter = 
      filter === 'all' ? true :
      filter === 'low-stock' ? p.quantity > 0 && p.quantity <= p.reorderLevel :
      p.quantity === 0;
    return matchesSearch && matchesFilter;
  });

  const lowStockProducts = products.filter(p => p.quantity > 0 && p.quantity <= p.reorderLevel);
          ...
          <Card className="p-4 bg-red-50 border-l-4 border-red-500 mb-6">
            <div className="flex gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-700">{outOfStockProducts.length} products are out of stock</p>
              </div>
            </div>
          </Card>
        )}
                ...
                {filteredProducts.map(product => {
                  ...
                  return (
                    <tr key={product.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center font-semibold">₹{product.price.toFixed(2)}</td>
                      ...
                      <td className="px-4 py-3 text-center">{product.reorderLevel}</td>
